...
/* --- Fixed dimensions for Member tab ---------------------------------- */
const PRIMARY_DIM   = "member";   // Member
  ...
  const fetchData = useCallback(async (
    ...
    try {
      ...
      if (d) {
        ...
        toast.success(res.data?.message ?? "Member scoreboard loaded");
      }
    } catch (err) {
      const msg = (err as any)?.response?.data?.message ?? "Failed to fetch member scoreboard";
      console.error("Failed to fetch member scoreboard:", err);
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="relative z-50 flex items-center gap-2 flex-wrap">
        ...
        {/* Locked dropdown — Member (primary) */}
        <div
          title="Fixed: Member"
