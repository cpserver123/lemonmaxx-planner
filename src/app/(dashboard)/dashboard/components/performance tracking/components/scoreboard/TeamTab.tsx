...
/* --- Fixed dimensions for Team tab ------------------------------------ */
const PRIMARY_DIM   = "team";      // Team Leader
  ...
  const fetchData = useCallback(async (
    ...
    try {
      ...
      if (d) {
        ...
        toast.success(res.data?.message ?? "Team scoreboard loaded");
      }
    } catch (err) {
      const msg = (err as any)?.response?.data?.message ?? "Failed to fetch team scoreboard";
      console.error("Failed to fetch team scoreboard:", err);
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="relative z-50 flex items-center gap-2 flex-wrap">
        ...
        {/* Locked dropdown — Team Leader (primary) */}
        <div
          title="Fixed: Team Leader"
